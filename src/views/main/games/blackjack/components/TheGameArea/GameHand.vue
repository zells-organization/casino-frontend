...
export default {
  ...
  computed: {
    isActiveHand () {
      return this.$store.state.blackjack.activeHandIndex === this.index
    },
    isInactiveHand () {
      return this.isSplit && !this.isActiveHand && this.$store.state.blackjack.activeHandIndex && this.index > 0
    },
    handClasses () {
      let classes = []
