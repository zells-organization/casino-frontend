<template>
    ...
    >
      ...
      <small>&times;</small>
      <span class="number">{{ $store.state.blackjack.bank }}</span>
    </span>
</template>
...
export default {
  ...
  watch: {
    '$store.state.blackjack.bank': function (current, previous) {
      if (current > previous) {
        const { startingBank, minimumBet } = this.$store.state.blackjack.settings
