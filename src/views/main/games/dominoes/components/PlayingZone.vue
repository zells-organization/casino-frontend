...
export default {
    ...
    watch: {
        ...
        restPieces(){
            if (this.restPieces < 1){
                ...
                if (this.$store.state.dominoes.machineChoice < 1) {
        ...
        empty(){
            if (this.empty === true){
                let lastRight = this.$store.state.dominoes.board.length - 1
                this.newLock.push(this.$store.state.dominoes.board[0].value[0])
                this.newLock.push(this.$store.state.dominoes.board[lastRight].value[1])
        ...
        pass(){
            if (this.$store.state.dominoes.machineChoices < 1) {
                this.keepPlaying = false
                this.neitherWins = true
                this.claimVictory()
                this.start = true
    ...
    computed: {
        hand(){
            return this.$store.state.dominoes.hand
        },
        handLength(){
            return this.hand.length
        },
        machineHand(){
            return this.$store.state.dominoes.machineHand
        },
        machineHandLength(){
            return this.machineHand.length
        },
        restPieces(){
            return this.$store.state.dominoes.shuffledPieces.length
        }
    },
    ...
    methods: {
        ...
        initGame: function(){
            if (this.$store.state.dominoes.board.length > 0) {
        ...
        drawAgain(player) {
            ...
            if (this.$store.state.dominoes.shuffledPieces.length === 0) {
                this.draw = false
                this.stopDrawing = true
                this.upto = !this.upto
            }
        ...
        machinePlays(){
            if (this.keepPlaying === true) {
                ...
                if (this.$store.state.dominoes.board.length > 0){
                    ...
                    let one = this.$store.state.dominoes.machineHand.filter(d => d.value[0] === head)
                    if (one) choices.push(one)

                    let two = this.$store.state.dominoes.machineHand.filter(d => d.value[1] === head)
                    if (two) choices.push(two)

                    let three = this.$store.state.dominoes.machineHand.filter(d => d.value[0] === tail)
                    if (three) choices.push(three)

                    let four = this.$store.state.dominoes.machineHand.filter(d => d.value[1] === tail)
                    ...
                    if (machineChoices.length === 0) {
                        if (this.$store.state.dominoes.noChoice === true){
                            this.keepPlaying = false

                        } else {
                            let continueDrawing = true
                            while (continueDrawing && this.$store.state.dominoes.shuffledPieces.length > 0) {
                                this.drawAgain(0)

                                let one = this.$store.state.dominoes.machineHand[this.$store.state.dominoes.machineHand.length-1]
        ...
        makeChoice(machineChoices, head, tail){
            ...
            let piece = this.$store.state.dominoes.machineHand.find(p => p.value[0] === final[0] && p.value[1] === final[1])
